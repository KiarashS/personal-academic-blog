...
const installable = pwaEnabled();

/*
 * What tells a browser the site can be installed. `mobile-web-app-capable` is
 * the standard name; Safari still reads the `apple-` one, so both are written.
 * The title is the launcher label iOS uses, which is the manifest's
 * `short_name` by another route — it reads the config rather than being typed
 * into index.html twice.
 */
const INSTALL_TAGS = [
...
function head({ title, description, url, type, image, feed, jsonLd }) {
  return (
    [
      ...
      // applied. The install tags travel with it: with `pwa.enabled` off there
...
// name changes whenever the built assets do. With `pwa.enabled` off neither is
// written: the site is a set of static pages again, and `lib/pwa-client.ts`
// retires the worker a returning reader still has.
...
if (installable) {
  await write('site.webmanifest', `${JSON.stringify(webManifest(), null, 2)}\n`);

  await write(
    'sw.js',
    `/* Generated by scripts/prerender.mjs — edit that, not this. */
const CACHE = '${CACHE_PREFIX}${buildStamp}';
