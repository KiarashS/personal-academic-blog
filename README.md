...
is fetched at any point. Thanks to Mayofont for the face; the full version and a
commercial licence are theirs to sell.

`scripts/set-signature.mjs` is what made it, and it needs your own copy of
whatever font you want the name written in:

```
node scripts/set-signature.mjs "~/fonts/Amsterdam Handwriting.ttf"
node scripts/trace-signature.mjs src/content/signature-typeset.svg 19
```
...
The drawing is fixed at 560 units wide rather than at a type size, because a
type size means nothing across fonts: an em is 2048 units in one face and 1000
in the next, and the same setting came out 584 units wide in one and 933 in
another. That matters twice. The mask width below is in user units, so it would
mean something different per font; and the tracer rasterises at six pixels per
unit, so a drawing half again as wide costs more than twice as much to thin.

Coordinates are rounded to a tenth of a unit, which at that width is three
hundredths of a pixel and saves a sixth of the file. Worth having, because the
file is inlined into the front page rather than fetched. How much there is to
inline is the face's doing and not the pipeline's: this one is 21KB and the
front page 31KB, where an ornate capital tried earlier was 65KB of outline on
its own.
...
a single connected component of ten thousand of them in one face tried here: it
overflowed the stack outright. The iterative walk produces the same strokes — re-tracing the traced
...
Work that is not a post and not a paper: a demo with its own scripts, a rendered
notebook, a D3 visualisation, a report exported from somewhere else. Each one is
a folder of plain files under `public/projects/`, served at the path it sits at:

```
public/projects/example/index.html   →   /projects/example/
```

Nothing in there goes through the build. No Markdown, no React, no bundling —
whatever is in the folder is what a reader gets, which is the point. A project
that needs its own copy of D3, or that was generated by a tool you do not
control, works unchanged.

Put whatever the project needs beside its page: more HTML, its own CSS and
JavaScript, JSON, images, subfolders. Vite copies `public/` verbatim, so the
files in `dist` are byte-for-byte the ones you wrote, and relative links between
them work as they read — `./assets/style.css` from `index.html` resolves to
`/projects/<name>/assets/style.css`. Prefer relative paths to site-absolute
ones: a site-absolute `/assets/style.css` is wrong the moment the site is served
from a subdirectory, and nothing rewrites paths inside these files.

`public/projects` is ignored by ESLint and Prettier, because a project's scripts
are frequently not yours and linting a vendored bundle against this project's
rules fails the build over code nobody is going to change. `npm run links` does
read the pages, though, so a relative link inside a project that points at
nothing fails the build like any other broken link.

They reach the site through a group in `nav`, which is a label with a list under
it rather than a link of its own:
...
no route for it and would answer with the 404 page. An entry with a scheme opens
in a new tab; one that is a path on this site does not. `npm run links` still
checks the internal ones against `dist`, so a folder you have not added yet
fails the build rather than shipping a dead nav entry.

A group is gated like anything else, and gating reaches inside it: entries whose
own feature is off are dropped, and a group left with nothing to label is
dropped with them.
