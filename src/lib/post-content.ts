...
const bodies = new Map<string, Resource<string>>();

/**
 * One post's body as a resource: suspending in the browser, and readable
 * without suspending once the build has warmed it. A rejection is dropped
 * rather than cached, which `resource` handles.
 */
export function postBody(slug: string): Resource<string> {
  const hit = bodies.get(slug);
  if (hit) return hit;

  const made = resource(() => loadPostHtml(slug));
  bodies.set(slug, made);
  return made;
}
